...
- **VISUAL audit caught a fidelity gap a green gate hid (Law 4).** test-flair-shell is green and verifies the FILE COPY modal structurally, but in the rendered PPM the modal BLENDS: window/dialog "white" is the gray 0x7F7F86, so a gray modal on gray windows is invisible. The oracle is right (structure present); the VISUAL shows the content-white gap. Bumped bead `bmih` (palette-unify + content-white) to P1.
