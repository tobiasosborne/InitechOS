...
> **AMENDMENT 2026-07-10 (bead initech-44ab, unanimous three-lens committee; follow-up rider 2026-07-11).** The D-5 cap values above are amended — this note records the change; the ratified text above is preserved as written. `RGN_ROW_X_MAX` **256 -> 640**: 640 is EXACTLY the ADR-0004 OD-3 640px coordinate-domain bound (640px => at most 320 disjoint spans => 640 inversion points), and — the load-bearing fact (RIDER-2 derivation, proven in `region.c` `xmerge`) — a single band's MERGED output is bounded by the number of DISTINCT x positions in the two input rows (coincident A/B edges collapse at emit; no-change double-toggles emit nothing), hence by the same 640 domain bound, NOT by the loose `na+nb`. The pre-amendment value (256) contradicted its own stated worst case: a legitimate full-width scanline was unrepresentable (fail-loud after the initech-mswo P0 fix). `RGN_X_POOL_CAP` **1024 -> 2048** in the same act: one maximally dense row is now 640 slots, and a single dense row plus ~100 typical rows (~4 pts each) already exceeded 1024. `RGN_ROWS_CAP` unchanged. The per-band bound and the pool bound are DIFFERENT quantities — one does not follow from the other; `spec/region_algebra.h` Sec 5 carries the two-bounds derivation verbatim. Follow-up riders (2026-07-11, stack-audit tripwire, rounds 2-3): `region_from_rects`' working regions (18.1 KiB of stack at the new caps, vs the single 64 KiB kernel stack shared with the IRQ handlers) and the three query helpers' working regions moved off the stack into a guarded, DUAL-MODE working set in `region.c` — hosted builds carry two static full-cap slots; freestanding kernels bind two FLAIR-heap-backed `rgn_ws_slot_t` blocks (`FLAIR_CLASS_REGION`, per ADR-0004 DEC-03) via the freestanding-only `region_engine_bind_ws()`, because a plain static array (round 2) busted the largest kernel variant's (`kernel_shell`) `_kernel_end < PROGRAM_BASE` link assert by 8,340 B. Fail-loud in-use guard (nested acquire panics; unbound slots panic — W1); `region_from_rects`' single-rect scratch shrank to the 2-row/2-slot `region_complement` stack pattern (W2); boot order `flair_heap_init -> bind_ws -> region_engine_reset -> shell_build_scene` is load-bearing because `kstart.asm` does not zero `.bss`. New oracle legs: a deterministic full-width 640px pixel-exact homomorphism leg + at-cap boundary round-trip + guard/leak probes (`test_region.c`); the mswo ASAN `NO_XMERGE_CAP` mutant auto-scales (`NSPANS = cap/2`) and was re-verified RED at the new cap.
