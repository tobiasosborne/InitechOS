...
;        (PROGRAM_IMAGE = 0x00020100 -- the org below MUST equal it).
...
;            linked into the kernel .rodata; the loader copies it to 0x20100.
;
; ADDRESSING (Sec 5.3 Solution A / Risk 2): the loader places this image at the
; FIXED address PROGRAM_IMAGE = 0x00020100. We assemble with `org 0x00020100`
...
bits 32
org 0x00020100                 ; == spec/memory_map.h PROGRAM_IMAGE (Sec 5.3)
...
start:
    ...
    ; `msg` is an absolute address (org 0x20100) so EDX is correct at load.
    mov ah, 0x09
    mov edx, msg
    int 0x21
